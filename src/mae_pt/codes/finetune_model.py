...
class ModelFinetuner(ModelPretrainer):
    ...
    def set_pretrained_mae(
        ...
        if not double_ft:
            ...
            self.model.mae.load_state_dict(state_dict)
        else:
            fc_state_dict = {}
            head_state_dict = {}
            ...
            for key in old_keys:
                ...
                if new_key.startswith("head."):
                    new_key = new_key[5:]
                    head_state_dict[new_key] = state_dict.pop(key)
                elif new_key.startswith("fc."):
                    new_key = new_key[3:]
                    fc_state_dict[new_key] = state_dict.pop(key)
                else:
                    state_dict[new_key] = state_dict.pop(key)
            self.model.mae.load_state_dict(state_dict)
            self.model.head.load_state_dict(head_state_dict)
        ...
        self.model.mae.to(self.args.device)
